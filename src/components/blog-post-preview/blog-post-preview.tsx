...
function BlogPostPreview(props: {
    ...
    if (post) {
        return (
            <div key={post._id} className={style.swiperSlider}>
                <div
                    style={{
                ...
                <div className={style.tags}>
                    {
                        post.data?.tags ? post.data?.tags.map((item: any, index: number) => {
                            return (<div key={index} className={style.tag}>{item}</div>)
                        }) : ""
                    }
                </div>
                <div>
                    <h2><Link href={`courses/${post._id}`}>{post.data?.title}</Link></h2>
                ...
                <div className={style.blogInfo}>
                    <div>
                        ...
                        <p>{formatNumbers(parseInt((post.data?.total_enrolled_students || '').replace(/,/g, '')))}</p>
                    </div>
