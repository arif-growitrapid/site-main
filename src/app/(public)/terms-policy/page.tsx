...
export const metadata: Metadata = {
  ...
  description:
    "Welcome to the Terms and Policies page of GrowItRapid - your source for information on our policies and procedures. Here, you'll find all the legal documents that govern your use of our website and services.",
  openGraph: {
    type: "website",
    title: "Terms & Policies",
    description:
...
export default async function page({}: {}) {
  const data =
    await clientFetch(groq`*[ _type == "terms-policies" ] | order(sno asc) {
        ...,
        items[]{
            title,
            "slug": slug.current,
    ...
    <div>
      ...
      >
        ...
        <div className={`relative z-20 px-6 py-24 md:max-w-5xl mx-auto`}>
          <h1
            className={`md:max-w-[50%] text-5xl text-center md:text-left leading-tight font-semibold text-[var(--dark-text-color)] md:text-current`}
          >
            Our Terms & Policies
          </h1>

          <p
            className={`mt-4 text-center md:text-left text-[var(--dark-text-color)] md:text-current max-w-[500px]`}
          >
            Transparency & Trust: Discover Our Policies and Terms for a Seamless
            Partnership
          </p>
        </div>
          ...
          {data.map((item: any, index: number) => (
            <div key={index} className={``}>
              <div className={``}>
                ...
                <p
                  className={`md:text-lg text-[var(--primary-color)] leading-relaxed`}
                >
                  {item.tagline}
                </p>
              </div>
              ...
              <div
                className={`flex flex-row flex-wrap justify-start items-stretch px-8 py-8`}
              >
                {item.items.map((item: any, index: number) => (
                  <Link
                    key={index}
                    href={`${config.links.terms_policy}/${item.slug}`}
                    className={``}
                  >
                    <div
                      className={`
