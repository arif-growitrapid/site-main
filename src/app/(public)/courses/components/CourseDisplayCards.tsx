...
const CoursesDisplayCard: React.FC<CourseCardProps> = ({ courses, type, titleNeeded, bgColor, swapTheme }) => {
  const navigationPrevRef = useRef<HTMLDivElement>(null)
  const navigationNextRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    console.log(courses, "CAT")
  }, [courses])

  const skeletonThemeProps = {
    baseColor: swapTheme ? 'var(--tertiary-color)' : 'var(--bg-color)',
    highlightColor: swapTheme ? 'var(--bg-color)' : 'var(--tertiary-color)',
  };
        ...
        <div className={style.popularBlogs}>
          <SkeletonTheme {...skeletonThemeProps}>
            {courses?.map((element, index) => (
              <div key={index} style={{backgroundColor: bgColor || 'initial'}} className={`${style.horizontalCards}`}>
                {element ? (
                ...
                <div className={style.content}>
                  <div className={style.profileContainer}>
                    {element ? (
                      <Image className={style.profilePic} width={50} height={50} src={element.instructors.image} alt='profile pic' />
                    ) : (
                      <Skeleton width={50} height={50} circle={true} style={{ marginRight: '1em' }} containerClassName={style.profilePic} />
                    )}

                    <div className={style.profileInfo}>
                      <div className={style.authorName}>{element?.instructors.name || <Skeleton width={`${Math.floor(Math.random() * (150 - 50 + 1)) + 50}px`} />}</div>
                      <div className={style.subtext}>{element ? element?.duration.toLowerCase() + ' • ' + element?.catalogs.length + ' lessons' + ' • ' + formatNumbers(element?.total_enrolled_students) + " students enrolled" : <Skeleton width={`${Math.floor(Math.random() * (150 - 50 + 1)) + 50}px`} />}</div>
                    </div>
                  </div>
        ...
        >
          {courses.map((element, index) => (
            <SwiperSlide key={index} className={style.card}>
              {element ? (
                <Image width={1080} height={720} src={element?.thumbnail} className={style.thumbnailCard} />
              ) : (
                ...
                {element ? (
                  ...
                  <div className={style.loader}>
                    {[...Array(5)].map((element, index) => (
                      <span key={index} className={style.tag}>
                        <Skeleton width={`${Math.floor(Math.random() * (100 - 20 + 1)) + 20}px`} containerClassName={style.tag}></Skeleton>
                      </span>
                    ))}
                  </div>
                )}
              ...
              <p className={style.desc}>{element?.description || <Skeleton count={3} />}</p>
              {element ? (
                <Link href={`/courses/${element?.meta.slug}`} className={style['learn_more_btn']}>
                  <div>
                    <span>Enroll Now</span>
              ...
              <div className={style.blogInfo}>
                <div>
                  <IoTimeSharp size={25} className={style.icon} />
                  <p>{element ? formatNumbers(element.duration) + "hrs" : ""}</p>
                </div>

                <div>
                  <PiStudentFill size={25} className={style.icon} />
                  <p>{element ? formatNumbers(element.total_enrolled_students) : ""}</p>
                </div>

                <div>
                  <MdWorkHistory size={25} className={style.icon} />
                  <p>{element ? formatNumbers(element.guarantee_percentage) + "%" : ""}</p>
                </div>
              </div>
