...
export default function Page({
  ...
  useEffect(() => {
    async function getCourseDetails() {
      try {
        const { type, data: courseData } = await getCourseById(
          "coursera",
          slug
        );
            ...
            <div className={style.parent}>
              ...
              <div className={style.btns}>
                <button>
                  ...
                  <Link href={courseData.redirectLink || ""}>
                    About The Course
                  </Link>
                </button>
                <button>
              ...
              <div className={style.numbers}>
                <div>
                  <h2 className={style.numeric}>
                    {courseData.guarantee_percentage || "68%"}
                  </h2>
                ...
                <div>
                  <h2 className={style.numeric}>
                    {formatNumbers(
                      (courseData.avg_salary.match(/[0-9,]+/) || [])[0] || "80k"
                    ) + "k" || "80k"}
                  </h2>
                  <p className={`line-clamp-2 ${style.text}`}>
                    Avg $ Salary Earned
                ...
                <div>
                  <h2 className={style.numeric}>
                    {formatNumbers(
                      (courseData.total_enrolled_students.match(/[0-9,]+/) ||
                        [])[0]?.replace(/,/g, "") || 0
                    ) || "1.2k"}
                  </h2>
                  <p className={`line-clamp-2 ${style.text}`}>
                ...
                <div>
                  ...
                  <p className={`line-clamp-2 ${style.text}`}>
                    Rating by {courseData.reviews || "42k"}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className={style.courseInfo}>
          <div className={style.eBooks}>
            ...
            <ul className={style.points}>
              {courseData?.what_you_will_learn.map(
            ...
            <div className={style.perks}>
              <div className={style.eBooks}>
                <div>
                  ...
                  >
                    <path
                      d="M2 24.4966C2 22.8397 3.34315 21.4966 5 21.4966H14.7114V41.6352H5C3.34315 41.6352 2 40.2921 2 38.6352V24.4966Z"
                      stroke="white"
                      stroke-width="4"
                    />
                    <path
                      d="M28.2891 5C28.2891 3.34314 29.6322 2 31.2891 2H38.0004C39.6573 2 41.0004 3.34315 41.0004 5V38.6352C41.0004 40.2921 39.6573 41.6352 38.0004 41.6352H28.2891V5Z"
                      stroke="white"
                      stroke-width="4"
                    />
                    <path
                      d="M15.1445 15.2124C15.1445 13.5555 16.4877 12.2124 18.1445 12.2124H24.8559C26.5127 12.2124 27.8559 13.5555 27.8559 15.2124V41.6351H15.1445V15.2124Z"
                      stroke="white"
                      stroke-width="4"
                    />
                  </svg>
          ...
          <div className={`${style.eBooks} ${style.catalog}`}>
            <h1>
              Specialization - {courseData.series.length} Course Series ??
            </h1>
            <div className={style.accordion}>
              {courseData.series.map((item, index: number) => (
                <div key={index} className={style["accordion-item"]}>
                  ...
                  >
                    <div>
                      ...
                      <div className={style.tags}>
                        <span
                          key={0}
                          className={`inline-block px-2 py-1 mr-2 text-[.65rem] font-semibold rounded-md bg-[var(--bg-color)] text-[var(--text-color)]`}
                        >
                          Course {index}
                        </span>
                        <span
                  ...
                  >
                    ...
                    <div className={style.tags}>
                      {item.internalTags.map((text: String, index: number) => {
                        return (
                          <span
                            key={0}
                            className={`inline-block px-2 py-1 mr-2 text-[0.6rem] font-semibold rounded-md bg-[var(--bg-color)] text-[var(--text-color)]`}
                          >
                            {text}
                          </span>
                        );
